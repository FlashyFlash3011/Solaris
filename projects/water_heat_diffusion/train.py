...
def train(args):
    ...
    train_loader = DataLoader(
        train_ds, args.batch_size, shuffle=True, pin_memory=(device.type == "cuda"), num_workers=2
    )
    val_loader = DataLoader(
        val_ds, args.batch_size, pin_memory=(device.type == "cuda"), num_workers=2
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if vl < best:
            best = vl
            save_checkpoint(Path(args.checkpoint_dir) / "best_fno.pt", model, opt, sched, epoch, vl)
