...
def load_simulation_data(data_path: str) -> dict:
    ...
    out = {
        k: d[k] for k in d.files if k not in ("flood_lower", "flood_upper", "track", "storm_pos")
    }
...
def run_visualization(
    data: dict, output: str, fps: int = 4, dpi: int = 130, interactive: bool = False
) -> None:
    import matplotlib
    ...
    gs = gridspec.GridSpec(
        2, 2, hspace=0.42, wspace=0.24, left=0.05, right=0.95, top=0.89, bottom=0.06
    )
    ...
    def _add_terrain(ax, alpha=1.0):
        ax.imshow(terrain_rgba, origin="lower", extent=ext, aspect="auto", alpha=alpha, zorder=1)
    ...
    _add_coast(ax_f)
    _add_colorbar(
        fig, ax_f, plt.cm.ScalarMappable(norm=flood_norm, cmap=flood_cmap), "Flood depth (m)"
    )
    flood_title = ax_f.set_title(
        "FLOODING MAP  —  t = 0.0 h", color="white", fontsize=10, pad=5, fontweight="bold"
    )
    ax_f.text(
        0.02,
        0.02,
        "White line = coastline   ⊕ = storm eye",
    ...
    cb_w = _add_colorbar(
        fig, ax_w, plt.cm.ScalarMappable(norm=wind_norm, cmap=wind_cmap), "Wind speed (m/s)"
    )
    ...
    if vmax_wind > 33:
        ...
        cb_w.ax.text(
            ...
            fontsize=5.5,
            va="center",
            transform=cb_w.ax.transAxes,
        )
    wind_title = ax_w.set_title(
        "HURRICANE WINDS  —  t = 0.0 h", color="white", fontsize=10, pad=5, fontweight="bold"
    )
    ...
    if has_unc:
        ...
        _add_colorbar(
            fig, ax_c, plt.cm.ScalarMappable(norm=unc_norm, cmap=conf_cmap), "Uncertainty ± (m)"
        )
        conf_lbl = f"AI CONFIDENCE  —  90% coverage  —  q={q_hat:.1f} m"
        ax_c.text(
            0.02,
            0.02,
            "Green = AI is confident   Red = AI is unsure",
        ...
        _add_colorbar(
            fig, ax_c, plt.cm.ScalarMappable(norm=wind_norm, cmap="plasma"), "Wind speed (m/s)"
        )
        conf_lbl = "STORM INTENSITY"
        ax_c.text(
            0.02,
            0.02,
            "Brighter = stronger, more dangerous winds",
            transform=ax_c.transAxes,
            color="0.65",
            fontsize=6.5,
            zorder=12,
    ...
    conf_title = ax_c.set_title(conf_lbl, color="white", fontsize=10, pad=5, fontweight="bold")

    # ── Panel 4 : Total damage ────────────────────────────────────────
    _add_terrain(ax_d)
    damage_im = ax_d.imshow(
        cumul_mask.astype(float),
        origin="lower",
        extent=ext,
        aspect="auto",
        cmap=damage_cmap,
    ...
    _add_coast(ax_d)
    damage_title = ax_d.set_title(
        "TOTAL FLOODED AREA  —  0.0% of coast", color="white", fontsize=10, pad=5, fontweight="bold"
    )
    ax_d.text(
        0.02,
        0.02,
        "Red = flooded at any point during the storm",
    ...
    bar_fill = mpatches.Rectangle((0, 0), 0.0, 1, facecolor="#4fc3f7", edgecolor="none")
    ...
    def _place_eye(ax, x_km, y_km, container):
        if container[0] is not None:
            try:
                ...
                pass
        c = mpatches.Circle((x_km, y_km), 3.2, fill=False, edgecolor="white", lw=2.0, zorder=10)
        ax.add_patch(c)
    ...
    def update(frame: int):
        ...
        damage_title.set_text(f"TOTAL FLOODED AREA  —  {pct:.1f}%  ({km2:.0f} km2)")
    ...
    if interactive:
        ...
        if sfx == ".gif":
            ...
            anim.save(str(out_path), writer=animation.FFMpegWriter(fps=fps, bitrate=2400), dpi=dpi)
        else:
