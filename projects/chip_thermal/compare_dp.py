...
def load_post_dp_model(
    ckpt_path: str, device: torch.device, Q_mean: float, Q_std: float, T_mean: float, T_std: float
):
...
def _infer_batched(
    model, Q_t: torch.Tensor, device: torch.device, batch_size: int = 64
) -> np.ndarray:
...
def run_comparison(args):
    ...
    if policy is not None and post_type == "residual":
        ...
        if not policy._fitted:
            ...
            policy.fit_from_model(
                post_model, _make_val_loader(Q_t, T_fd, T_mean, T_std), device, n_samples=200
            )
    ...
    log.info(
        f"  Pre-DP  FNO : {pre_time * 1000:.0f}ms  ({pre_ms:.2f} ms/layout)  "
    ...
    log.info(
        f"{post_tag}: {post_time * 1000:.0f}ms  ({post_ms:.2f} ms/layout)  "
...
def _plot(
    ...
    gs = gridspec.GridSpec(
        2, 4, figure=fig, wspace=0.38, hspace=0.52, left=0.04, right=0.97, top=0.89, bottom=0.04
    )
    ...
    im = axes[0][1].imshow(
        T0_fd, cmap="inferno", origin="lower", vmin=vmin_T, vmax=vmax_T, interpolation="bilinear"
    )
    axes[0][1].set_title(
    ...
    im = axes[0][2].imshow(
        T0_pre, cmap="inferno", origin="lower", vmin=vmin_T, vmax=vmax_T, interpolation="bilinear"
    )
    axes[0][2].set_title(
    ...
    im = axes[0][3].imshow(
        T0_post, cmap="inferno", origin="lower", vmin=vmin_T, vmax=vmax_T, interpolation="bilinear"
    )
    axes[0][3].set_title(
    ...
    im = axes[1][2].imshow(
        err_pre, cmap="RdBu_r", origin="lower", vmin=0, vmax=err_max, interpolation="bilinear"
    )
    axes[1][2].set_title(
    ...
    im = axes[1][3].imshow(
        err_post, cmap="RdBu_r", origin="lower", vmin=0, vmax=err_max, interpolation="bilinear"
    )
    axes[1][3].set_title(
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--pre_checkpoint",
        default=str(_here / "checkpoints/best_fno.pt"),
        help="Pre-DP (plain FNO) checkpoint",
    )
    p.add_argument(
        "--post_checkpoint",
        default=str(_here / "checkpoints/best_residual.pt"),
        help="Post-DP model checkpoint (residual or constrained)",
    )
    p.add_argument("--device", default="cuda")
    p.add_argument(
        "--n_batch", type=int, default=1000, help="Number of new chip layouts to benchmark"
    )
    p.add_argument(
        "--batch_size", type=int, default=64, help="GPU batch size for neural model inference"
    )
    p.add_argument(
        "--seed",
        type=int,
