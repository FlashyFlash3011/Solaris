...
def train(args) -> None:
    ...
    val_ds = TensorDataset(
        fi_t[n_train : n_train + n_val],
        ft_t[n_train : n_train + n_val],
        wi_t[n_train : n_train + n_val],
        wt_t[n_train : n_train + n_val],
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for fi, ft, wi, wt in train_loader:
            ...
            loss = flood_loss(
                outputs["flood"], ft, outputs["wind"], wt, args.alpha_wind, args.alpha_cons
            )
            loss.backward()
            clipper(model)
            optimizer.step()
    ...
    log.info(
        f"Coverage: {report['coverage']:.3f}  interval width: {report['mean_interval_width']:.4f}"
    )
