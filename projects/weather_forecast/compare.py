...
def afno_forecast(model, z0, t0, lead_days, meta, device):
    ...
    for day in lead_days:
        ...
        x = torch.as_tensor(np.stack([z0_n, t0_n, lt[0]])[None], dtype=torch.float32).to(device)
        with torch.no_grad():
...
def make_figure(
    z0, t0, z_sol, t_sol, z_afno, t_afno, lead_days, solver_time, afno_time, out_path, lat, lon
):
    ...
    outer = gridspec.GridSpec(5, 1, figure=fig, height_ratios=[0.6, 1, 1, 1, 0.55], hspace=0.12)
    ...
    ax0.set_title(
        "Initial state — Day 0  (z500 geopotential height)", color="white", fontsize=11, pad=5
    )
    ...
    for row in range(3):
        ...
        for col in range(n_leads):
            ...
            if row == 0:
                ax.set_title(f"Day {lead_days[col]}", color="white", fontsize=10, pad=4)
            if col == 0:
                ax.set_ylabel(row_labels[row], color=row_colors[row], fontsize=9, labelpad=6)
...
def run(args):
    ...
    log.info(
        f"AFNO | {model.num_parameters():,} params | {n_days}-day forecast on {nlat}×{nlon} grid"
    )
