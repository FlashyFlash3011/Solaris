...
def _laplacian_norm(field: np.ndarray) -> np.ndarray:
    ...
    return (
        np.roll(field, 1, axis=1) + np.roll(field, -1, axis=1) + fp[:-2, :] + fp[2:, :] - 4 * field
    )


def _advect(field: np.ndarray, u: np.ndarray, v: np.ndarray, dt: float, dx: float) -> np.ndarray:
    """First-order upwind advection: ∂f/∂t = -u·∂f/∂x - v·∂f/∂y.
    u, v in grid-cells/second (= m/s / dx_m).
    """
    dfx = np.where(u > 0, field - np.roll(field, 1, axis=1), np.roll(field, -1, axis=1) - field)
    fp = np.pad(field, ((1, 1), (0, 0)), mode="reflect")
...
def step_model(
    ...
    U = (
        0.3
        * dt_hours
        * (np.exp(-((phi - 0.785) ** 2) / 0.15) + np.exp(-((phi + 0.785) ** 2) / 0.15))
    )
...
def simulate(
    ...
    snap_at = {int(round(i * (n_steps - 1) / (n_snapshots - 1))): i for i in range(n_snapshots)}
